...
FS_t fs_create_child(FS_t parent, char *path, uint8_t allow_rm) {
    ...
    if (!p) { parent->child_next = child; return child; }
...
FS_t getFSChild(FS_t parent, char *path) {
    FS_t p = parent->child_next;
    while (p) { if (strcmp(p->path, path) == 0) return p; p = p->level_next; }
    return NULL;
}
...
static void ram_rm_file(DrTNode_t device) {
    ...
    while (p) {
        if (p == device) { prev->next = p->next; kernel_free(p); return; }
        prev = p; p = p->next;
    }
}
...
void ram_rm(char *aim_dir) {
    ...
    strcpy(p1, aim_dir); strcpy(p2, aim_dir);
...
void ram_touch(char *full_path, void *data, uint16_t data_len) {
    ...
    FS_t parent;
    if (!filename) { parent = currentFS; filename = path; }
    else { *filename = '\0'; filename++; parent = (*path) ? loadPath(path) : RAM_FS; }
    if (!parent) { USB_printf("touch: parent not found\n"); kernel_free(path); return; }

    if (getFSChild(parent, filename)) { USB_printf("touch: '%s' exists\n", filename); kernel_free(path); return; }
    DrTNode_t d = parent->node;
    while (d) { if (strcmp(d->name, filename) == 0) { USB_printf("touch: '%s' exists\n", filename); kernel_free(path); return; } d = d->next; }

    DrTNode_t file = (DrTNode_t) kernel_alloc(sizeof(struct DrTNode));
    file->name = kernel_alloc(strlen(filename) + 1); strcpy(file->name, filename);
    file->description = kernel_alloc(8); strcpy(file->description, "file");
    file->type = DrTFILE; file->status = DEVICE_ON;
    file->device = NULL; file->fops = NULL;
    file->parent = parent; file->next = NULL;
    file->data = kernel_alloc(128); memset(file->data, 0, 128);
    if (data && data_len > 0) memoryCopy(file->data, data, data_len > 127 ? 127 : data_len);
    Mutex_t m = (Mutex_t) kernel_alloc(MUTEX_SIZE); mutex_init(m); file->mutex = m;
    DrTNode_t p = parent->node;
    if (!p) parent->node = file;
    else { while (p->next) p = p->next; p->next = file; }
    parent->node_count++;
    kernel_free(path);
}
...
void ram_ls(char *path) {
    ...
    FS_t c = node->child_next;
    while (c) { USB_color_printf(LIGHT_GREEN, "%s  ", c->path); c = c->level_next; }

    DrTNode_t d = node->node;
    while (d) { USB_color_printf(LIGHT_BLUE, "%s  ", d->name); d = d->next; }
    ...
    while (t) { USB_color_printf(LIGHT_PURPLE, "%s\t", t->name); t = t->next; }
    USB_printf("\n");
}
...
FS_t ram_cd(char *path) {
    if (path[0] != '/' && currentFS->sd_mount_path) {
            ...
            if (slash) {
                *slash = '\0';
                if (strcmp(base, currentFS->sd_mount_path) == 0) {
                    if (currentFS->sd_cd_path) { kernel_free(currentFS->sd_cd_path); currentFS->sd_cd_path = NULL; }
                    *slash = '/';
                } else {
        ...
        DIR d; if (f_opendir(&d, new_sd) == FR_OK) { f_closedir(&d);
            if (currentFS->sd_cd_path) kernel_free(currentFS->sd_cd_path);
            currentFS->sd_cd_path = new_sd; return currentFS;
        }
        kernel_free(new_sd); return currentFS;
    }
    if (path[0] == '/' && currentFS->sd_mount_path) {
        if (currentFS->sd_cd_path) { kernel_free(currentFS->sd_cd_path); currentFS->sd_cd_path = NULL; }
    }
...
static void pwd(FS_t fs, char *path) {
    if (fs == RAM_FS) { strcpy(path, "/"); return; }
    pwd(fs->parent, path);
    strcat(path, fs->path);
    strcat(path, "/");
}
