...
因QSPI的外部flash仅有16MB，所以在文件系统中使用了RAMFS文件系统，将文件系统放在SDRAM中，这样可以更好的利用QSPI的flash。同时所有文件放入SD卡中，这样就可以实现文件系统的功能。

文件夹通过mkdir创建后，利用mount挂载SD卡的目录，这样内存空间就会大的离谱。同时通过ls命令可以查看文件夹下的文件，通过cd命令可以切换目录，通过cat命令可以查看文件内容。
